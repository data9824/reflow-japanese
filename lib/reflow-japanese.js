...
export default {
	...
	reflow() {
		...
		if (editor = atom.workspace.getActiveTextEditor()) {
			...
			let out = this.reflowText(lineText, editor.getTabLength(), lineLength);
			editor.moveRight();
	...
	reflowText(lineText, tabLength, lineLength) {
		...
		if (match !== null) {
			lineText = lineText.substr(MARK_WIDTH);
			firstIndent = indent + match[0];
			indent += '  ';
			indentWidth += MARK_WIDTH;
		} else {
			firstIndent = indent;
		}
